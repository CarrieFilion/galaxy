...
c externals
      logical gtlogl
      logical jscren
      real grofu, rofu, roundup, version
c
c local arrays
      logical act( mcmp )
      integer ibuf
...
c switch to grid with largest rgrid
      ...
      sp = 1
      if( all )then
   2    call gtchar( 'Enter component number to plot or all?', a )
        call lowercase( a )
        all = a .eq. 'all'
        if( .not. all )then
          read( a, * )sp
          if( .not. act( sp ) )go to 2
          do i = 1, icmp
            act( i ) = i .eq. sp
          end do
          call switch( igrd( sp ) )
      ...
      if( .not. all )print *, 'population', sp, ' selected'
c
      rmx = 0
      if( twod )then
...
c other grid types
        ...
        rmx = rgrid( jgrid ) / lscale
        print *, 'rmx =', rmx
c        if( gtlogl( 'Do you want to change rmx' ) )call gtreal(
c     +                                 'Enter rmx in disk scales', rmx )
        xmx = rmx
        ymx = rmx
        zmx = zm( jgrid ) / lscale
        if( positn )then
...
c determine whether to shift the frame with the grid
      if( centrd .and. ( positn .or. meriod ) )then
...
c z height too large
          ...
          if( j .gt. 0 )then
            print '( ''Plotting run'', i5, '' step no'', i6, i8, ' //
     +            ' '' particles'' )', irun, istep, j
c write heading
