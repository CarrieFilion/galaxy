...
c
c local arrays
      integer displs( mnodes ), nbuf( mnodes )
c
c local variables
      character*4 bstr( 2 )
...
c gather the counts from each processor
          call mpi_allgather( n, 1, mpi_integer, nbuf, 1,
     +                        mpi_integer, mpi_comm_world, i )
          displs( 1 ) = 0
          do i = 2, numprocs
            displs( i ) = displs( i - 1 ) + nbuf( i - 1 )
          end do
c copy the data on this node only to a temporary buffer
          allocate ( w( n ) )
          call blkcpy( prfrad, w, n )
c remember n on this node
          j = n
c gather the data onto the master only
          call mpi_gatherv( w, n, mpi_real, prfrad, nbuf, displs,
     +                                  mpi_real, 0, mpi_comm_world, i )
          n = displs( numprocs ) + nbuf( numprocs )
          deallocate ( w )
        end if
c
        if( n .gt. mprho )then
          lw = 2 * ( ( n - 1 ) / mprho + 1 )
          if( parallel .and. uqmass )lw = lw + j
          allocate ( w( lw ) )
          if( master )then
...
c combine data
            if( parallel )then
c copy the data to a temporary buffer
              if( n + j .gt. lw )then
                if( master )print *, n + j, lw
                call crash( 'RHOPRF', 'Work space too small' )
              end if
              do i = 1, n
                w( j + i ) = prfrad( i )
              end do
c gather the data onto the master only
              call mpi_gatherv( w( j + 1 ), n, mpi_real, prfrad, nbuf,
     +                          displs, mpi_real, 0, mpi_comm_world, i )
              n = displs( numprocs ) + nbuf( numprocs )
            end if
