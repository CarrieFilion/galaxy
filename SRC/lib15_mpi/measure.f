...
c
c calling arguments
      integer jlen, lglen, llz, lmoni
      integer lprop, lzman, lzpr, nact, nbplot
...
c
        if( lval )then
          if( parallel )then
c gather results from other processors - Lzval is sparsely filled
            allocate ( w( nbod ) )
            call mpi_reduce( Lzval, w, nbod, mpi_real, mpi_sum,
     +                       0, mpi_comm_world, i )
c            allocate ( w( 2, nbod ) )
c            call mpi_reduce( Lzval, w, 2 * nbod, mpi_real, mpi_sum,
c     +                       0, mpi_comm_world, i )
            do i = 1, nbod
              Lzval( i ) = w( i )
c              Lzval( 1, i ) = w( 1, i )
c              Lzval( 2, i ) = w( 2, i )
...
c            write( nphys )( Lzval( 1, i ), Lzval( 2, i ), i = 1, nbod )
          end if
        end if
c
