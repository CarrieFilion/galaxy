...
c update accelerations of heavies
        if( firsth .and. ( jlist .gt. 1 ) )then
          if( parallel )then
            call crash( 'STEP', 'New code has not been tested' )
            k = 4 * ndrct
            allocate ( w( 4, ndrct ) )
            allocate ( x( 4, ndrct ) )
            do i = 1, ndrct
            ...
            call mpi_allreduce( w, x, k,
     +                          mpi_real, mpi_sum, mpi_comm_world, i )
            k = 0
            do i = 1, ndrct
              do j = 1, 4
                k = k + 1
                drpt( j + 7, i ) = drpt( j + 7, i ) + x( j, i )
              end do
            end do
            deallocate ( w )
            deallocate ( x )
...
c sum results from all processors if required
      if( parallel )then
        call mpi_allreduce( noffm, j, 1, mpi_integer, mpi_sum,
     +                                               mpi_comm_world, i )
        noffm = j
        call mpi_allreduce( nshortm, j, 1, mpi_integer, mpi_sum,
     +                                               mpi_comm_world, i )
        nshortm = j
        call mpi_allreduce( ncenm, ncenl, 4, mpi_integer, mpi_sum,
     +                                               mpi_comm_world, i )
        do i = 1, 4
          ncenm( i ) = ncenl( i )
        end do
        if( lsfp )then
          call mpi_allreduce( ncontrib, j, 1, mpi_integer, mpi_sum,
     +                                               mpi_comm_world, i )
          ncontrib = j
        end if
      end if
