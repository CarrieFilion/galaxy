...
c
c local variables
      integer i, ierr, im, ip, ipp, j, k, l, m, n, nbin, nn
...
c gather global integrals data
      if( parallel )then
        allocate ( w( 2 * lcheck ) )
        call mpi_reduce( check, w, lcheck, mpi_double_precision,
     +                                mpi_sum, 0, mpi_comm_world, ierr )
        if( master )call blkcpy2( w, check, lcheck )
        call mpi_reduce( nspop, w, nact, mpi_integer, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, nspop, nact )
        deallocate ( w )
      end if
c integrals
      if( master )then
...
c normalize and save particle integrals
      if( moni )then
        if( parallel )then
c number of monitored particles on each processor
          n = ( nmonit - 1 ) / numprocs + 1
          k = 6 * n
          if( master )allocate ( w( k * numprocs ) )
c gather results from all processors
          call mpi_gather( ehmon, k, mpi_real, w, k, mpi_real, 0,
     +                                               mpi_comm_world, i )
c rearrange back into ehmon array as if for a single processor
          if( master )then
            l = 0
            do i = 1, n
              m = 6 * ( i - 1 )
...
c save angular momentum data
      if( angm .and. parallel )then
        k = nact * nLbin
        allocate ( w( k ) )
        call mpi_reduce( nLa, w, k, mpi_integer, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, nLa, k )
        deallocate ( w )
      end if
...
c normalize and save spherical Bessel coefficients
      if( sphb .and. parallel )then
        k = 2 * nact * jlen
        allocate ( w( k ) )
        call mpi_reduce( npbess, w, nact, mpi_integer, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, npbess, nact )
        call mpi_reduce( besc, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, besc, k )
        deallocate ( w )
      end if
...
c save rings data
      if( rngs .and. parallel )then
        k = nrings * npring * ( ncoor / 2 )
        allocate ( w( k ) )
        call mpi_reduce( wring, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, wring, k )
        deallocate ( w )
      end if
...
c normalize and save logarithmic spiral coefficients
      if( lgsp .and. parallel )then
        k = 2 * np * nm * nact
        allocate ( w( k ) )
        call mpi_reduce( alspi, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, alspi, k )
        deallocate ( w )
      end if
...
c normalize and save z mean analysis coefficients
      if( zanl .and. parallel )then
        k = 2 * nrz * nmz * nact
        allocate ( w( k ) )
        call mpi_reduce( zman, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, zman, k )
        deallocate ( w )
      end if
...
c normalize and save z-density profile of disk
      if( zprf .and. parallel )then
        k = nbzr * nbzz * nact
        allocate ( w( k ) )
        call mpi_reduce( zpbin, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, zpbin, k )
        deallocate ( w )
      end if
...
c
      if( ( vfld .or. vflh ) .and. parallel )then
        k = max( idskp * ndrad, ihskp * nhrbin )
        k = k * nact
        allocate ( w( k ) )
        call mpi_reduce( prop, w, k, mpi_real, mpi_sum, 0,
     +                                            mpi_comm_world, ierr )
        if( master )call blkcpy( w, prop, k )
        deallocate ( w )
      end if
