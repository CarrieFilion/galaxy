...
c local variables
      ...
      logical dfuqm, euler, rflsym, lh, luqmass
      real a, amp0, cph, cps, cth, phfc, phbase, sph, sps, sth, vrad
      real vscale, vtan, vx, vy, vz, wp, wpm, wtp, x, xscale, y, z
...
c set npr, if not preset
     ...
     +            sf3d )npr( icmp ) = ( 2 * ng - 1 ) / nsect + 1
              ...
              if( p3d )then
                npr( icmp ) = ( 2 * ng - 1 ) / nsect + 1
                rflsym = z0init( icmp ) .gt. 0.
                if( rflsym )npr( icmp ) = 4 * npr( icmp )
              end if
...
c remember what the DF file contains and restore the global parameter
     ...
     +                                    idim, irec, 'pbuf', 'PSETUP' )
            ...
            if( dfuqm )nwp = nwp + 1
c allow for a possible extra heavy particle
            i = nsp( icmp )
            if( lheavy .and. lh )i = i - 1
            if( ( norb * npr( icmp ) ) .ne. i )then
              if( master )print
     +               '( ''Expected'', i8, ''/'', i2, '', found'', i8 )',
     +                                              i, npr( icmp ), norb
              call crash( 'PSETUP',
...
c allow for a possible extra heavy particle
            i = nsp( icmp )
            if( lheavy .and. lh )i = i - 1
            if( mod( i, npr( icmp ) ) .ne. 0 )call crash( 'PSETUP',
...
c create a double ring of particles that can be reflection symmetric
            if( euler .or. rflsym )phfc = 2 * phfc
          end if
c
...
c distribute particles around ring
            phbase = 2. * pi * ranuni( 0. )
            do i = 1, npr( icmp )
              phi = phbase + phfc * real( i - 1 )
              if( .not. euler )then
c random nudge
...
c place second half of particles symmetrically on the other side of disk plane
                if( rflsym .and. ( i .eq. 1 + npr( icmp ) / 2 ) )then
                  z = -z
                  vz = -vz
                  psi = psi + pi
                  cps = cos( psi )
...
c add heavy anchor particle if required
          if( lheavy .and. lh )then
            if( is .gt. mbuff )then
              call relabl( mbuff )
              if( nx .gt. lpf )call crash( 'PSETUP',
     +                                        'Particle file overflow' )
...
c particle should be at rest in the center
            do i = 1, ndimen
              newc( i, is ) = 0
              newc( i + ndimen, is ) = 0
            end do
            ...
            loc( is ) = nx
            iflag( is ) = icmp
c make it 10 times more massive than any other in this population
            pwt( is ) = 10. * wpm
            label( is ) = igrd( icmp )
c on master node for now
            jebind( jgrid ) = 0
            loch( jgrid ) = nx
c increment counters
            nx = nx + nwpp
...
c   ==> if this is not done with individual particle masses
      if( .not. uqmass )then
        np1 = 0
        do i = 1, ncmp
          if( ( .not. rigidp( i ) ) .and. ( .not. testp( i ) ) )then
            np1 = np1 + 1
c remember the component number that has massive particles
            icmp = i
          end if
        end do
        if( np1 .gt. 2 )then
     ...
     +            'pmass adjustment required for multiple populations' )
        ...
        pmass = pmass * fmass( icmp )
        call slfset
      end if
c make sure icmp is undefined
      icmp = 0
      return
      end
