...
c
c It differs from the usual s3dacc routine by subtracting off the self-force
c   for each particle in turn, which is essential for particles in intensive
c   care, and maybe for other applications
      use aarrays
      implicit none
c
c calling argument
...
c local allocatable arrays
      real, allocatable, save :: Q( :,: ), rG( :,: ), sf( :,:,: )
      real, allocatable, save :: cmp( : ), smp( : )
c
c local variables
      integer i, is, j, k, l, lm, m, n, nrg
      logical firstc
      real arg, cp, f1, f2, f3, f4, rad, r2, s, sp, t, tiny, u, x, y, z
      parameter ( tiny = 1.e-10 )
      save firstc
      data firstc / .true. /
c
c copy positions of these particles on first call
      if( isubst .le. 1 )then
        do is = 1, jst
          do i = 1, ndimen
            oldpos( i, is ) = oldc( i, is )
          end do
        end do
      end if
c allocate space
...
c initialise
        ...
        i = 4 * s3ntm * ( k - 1 ) - 4
        j = 4 * s3ntm * k - 4
c get the mass contribution of this particle
        call s3self( is, sf )
        lm = 0
        do l = 0, s3lmax
          do m = 0, l
...
c combine coeffs from both grids if needed
              do n = 1, 2
                sf( n, 1, lm ) = s3dfld( i + n, 1 ) +
     +                           s3dfld( i + n, 2 ) - sf( n, 1, lm )
                sf( n, 2, lm ) = s3dfld( j + n, 1 ) +
     +                           s3dfld( j + n, 2 ) - sf( n, 2, lm )
                sf( n, 3, lm ) = s3dfld( i + n + 2, 1 ) +
     +                       s3dfld( i + n + 1, 2 ) - sf( n, 3, lm )
                sf( n, 4, lm ) = s3dfld( j + n + 2, 1 ) +
     +                       s3dfld( j + n + 2, 2 ) - sf( n, 4, lm )
              end do
            else
c subtract mass of this particle from the total on a single grid
              do n = 1, 2
                sf( n, 1, lm ) = s3dfld( i + n, jgrid ) - sf( n, 1, lm )
                sf( n, 2, lm ) = s3dfld( j + n, jgrid ) - sf( n, 2, lm )
                sf( n, 3, lm ) = s3dfld( i + n + 2, jgrid ) -
     +                                                    sf( n, 3, lm )
                sf( n, 4, lm ) = s3dfld( j + n + 2, jgrid ) -
     +                                                    sf( n, 4, lm )
              end do
            end if
            if( k .lt. nrg )then
c combine real and imaginary parts of interior and exterior masses
              do n = 1, 2
                Q( n, lm ) = wt( 1, is ) * sf( n, 1, lm ) +
     +                       wt( 2, is ) * sf( n, 2, lm ) +
     +                       wt( 1, is ) * sf( n, 3, lm ) +
     +                       wt( 2, is ) * sf( n, 4, lm )
                rG( n, lm ) =
     +                     -( l + 1 ) * ( wt( 1, is ) * sf( n, 1, lm ) +
     +                                    wt( 2, is ) * sf( n, 2, lm ) )
     +                            + l * ( wt( 1, is ) * sf( n, 3, lm ) +
     +                                    wt( 2, is ) * sf( n, 4, lm ) )
              end do
...
c particle beyond outer edge - all mass is interior and wt( 1, is ) = 0
              do n = 1, 2
                Q( n, lm ) = wt( 2, is ) * sf( n, 2, lm )
                rG( n, lm ) = -( l + 1 ) * wt( 2, is ) * sf( n, 2, lm )
              end do
            end if
          end do
        end do
...
c get Plms and their derivatives
        arg = z / rad
...
c        if( .not. lg( 1 ) )then
c          gpot( is ) = gpot( is ) - s3dfld( 1, 1 ) / rad
c          s = s3dfld( j + 1, 1 ) / rad**3
